...
sub _aggregate
{
	...
	{
		if (   $d->{CALL_SERVICE_CODE} =~ /^2[123]$|^8[1234689]$|^9[01]$/
			or (    $d->{CALL_SERVICE_CODE} =~ /^30$/ 
				and $d->{CALL_SERVICE_TYPE} =~ /^0$|^00$/
			   )
		   )
		{
	...
	if (defined $d->{CALL_BREAKDOWN_CODE})
	{
        ...
        elsif ($d->{CALL_BREAKDOWN_CODE} eq 'ROAMR' and $d->{CALL_SERVICE_CODE} eq '60' and $d->{CALL_SERVICE_TYPE} eq '51')
        {
            $ST_DATA = TRUE;
            $ST_DATA_GPRS_POSTPAID_TAP = TRUE; push @serviceTypes, "ST_DATA_GPRS_POSTPAID_TAP";
			$ST_UNIDENT = FALSE;
        }
	...
	if (defined $d->{CALL_PLMN_CODE} and $d->{CALL_PLMN_CODE} ne "IRLVF" and not $ST_DATA) {
		...
		if ($ST_SMS) {
			...
			elsif ( defined $d->{CALL_RECORD_TYPE} and $d->{CALL_RECORD_TYPE} eq '30' )
			{
