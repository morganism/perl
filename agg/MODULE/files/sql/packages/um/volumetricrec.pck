create or replace package volumetricrec is
...
/
create or replace package body volumetricrec is
  ...
  begin
    ...
    if mrec_def_id_list.count > 0 then
      ...
      for i in mrec_def_id_list.first .. mrec_def_id_list.last loop
        ...
        begin
          -- Batch it!
          retMsg := metricReconciliationBatcher(mrec_def_id,
                                                offset_days,
                                                num_days);
 ...
 -- global_debug := 1;
    ...
    start_date := trunc(sysdate - offset_days, 'HH24');
    end_date   := (sysdate - offset_days) + num_days;
    ...
    s_date := start_date;
    e_date := s_date + (rec_interval / (24 * 60));
    ...
    select count(1)
    into   v_multi_version
    from   um.mrec_version_ref v
    where  v.mrec_definition_id = the_mrec_ref.mrec_definition_id
    and    v.mrec_version_id != the_mrec_ref.mrec_version_id
    and    exists (select *
                   from   um.mrec f
                   where  f.mrec_version_id = v.mrec_version_id
                   and    f.d_period_id between sysdate -7 and sysdate);

    IF v_multi_version > 0 THEN
       global_multi_versions := 1;
    END IF;
    ...
    while s_date < end_date loop
      ...
      -- need to keep the agg chart data upto date as we, for the forecasts.
      if to_char(e_date,'HH24') = '00' then
      ...
      -- next block
      s_date := e_date;
      e_date := e_date + (rec_interval / (24 * 60)); -- as rec_interval is integer and s_date
      -- always starts on HH:00:00 boundary then
      -- e_date will maintain HH:00:00 boundary
  ...
  ************************************************/
  ...
  begin
    ...
    if new_data then
      ...
      MrecVersionDuplicateRecCheck(the_mrec_ref);
      ...
      -- LHS (-1)
      select f.d_period_id, sum(mrec) bulk collect
       ...
       group by f.d_period_id
       order by f.d_period_id;
  ...
  begin
    ...
    FOR p_dates in c_dates (start_date, end_date) LOOP
        ...
        FOR p_blank_forecasts IN c_blank_forecasts (p_dates.day_start, p_dates.day_end) LOOP
              ...
              WHERE  afmc.d_period_id = p_blank_forecasts.d_period_id
              AND    exists (select 1
                             from   um.d_mrec_line_mv d
                             where  d.mrec_definition_id = p_mrec_definition_id
                             and    d.d_mrec_line_id = afmc.d_mrec_line_id
  ...
  *******************************************************************/
  ...
  begin
    ...
    v_source_id := to_number(global_parameters('-source'));
     ...
     loggerForce('Source is : ' || v_source_id);
...
*/
  ...
  begin
    ...
    note_text := '<tr><td><table>' ||
                 ...
                 '</td></tr><tr><td>Forecast Value:</td><td>' || forecast_value ||
...
*/
  ...
  begin
    ...
    if global_multi_versions = 1 then
