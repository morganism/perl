...
PARTITION BY RANGE (D_PERIOD_ID)
    ( PARTITION LOG_RECORD_P00000000 VALUES LESS THAN (TO_DATE('20000101','yyyymmdd')))
...
ALTER TABLE LOG_RECORD
  ...
  USING INDEX LOCAL TABLESPACE UM_DWH_USG_IDX compress 1 ;
 
REM ----------------------------------------------
REM The linesbelow change the local index into a global index
...
CREATE INDEX LOG_RECORD_09_IDX ON LOG_RECORD(FILE_NAME)
  LOCAL
  TABLESPACE UM_DWH_USG_IDX
  nologging compress 1  ;
