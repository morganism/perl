package Ascertain::UM::VFI::Aggregator::DS95;
...
sub _aggregate
{
	...
	if ($ST_VOICE and defined $d->{discount_cent} and $d->{discount_cent} > 0.00) {
		$ST_VOICE_IN_BUNDLE = TRUE; push @serviceTypes, "ST_VOICE_IN_BUNDLE";
		$self->specifySourceForAggRecords("ST_VOICE_IN_BUNDLE",$provider);
	}
	elsif ($ST_VOICE and ( not   defined $d->{discount_cent} or $d->{discount_cent} == 0 )) {
		$ST_VOICE_OUT_BUNDLE = TRUE; push @serviceTypes, "ST_VOICE_OUT_BUNDLE";
        $self->specifySourceForAggRecords("ST_VOICE_OUT_BUNDLE",$provider);
	}
