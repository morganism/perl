package Ascertain::UM::VFI::Aggregator::DS96;
...
sub _aggregate
{
    ...
    #----------------- TIMESLOT --------------------
	my $datetime = (defined $d->{DATETIME}) ? $d->{DATETIME} : "";
    my $timeSlot = $datetime;
    ...
    if (defined $d->{BNUMBER} and $d->{BNUMBER} =~ /^00/ and $d->{BNUMBER} !~ /^00353/ ) {
