package uk.co.cartesian.vfi.um.web.servlets;
...
/**
 * VFI Specific implementation - VFI uses time based metrics. This class has been modified 
 * from the original so that the metric chart only returns values which are useful/meaningful to VFI.  
 * 
 * @author jeremy.sumner
 *
 */
public class VFIChartMetricSwfDataServlet
extends HttpServlet
{
    private static final long serialVersionUID = 1L;

    private static Logger logger = LogInitialiser.getLogger(VFIChartMetricSwfDataServlet.class.getName());
    ...
    {
        logger.debug("VFIChartMetricSwfDataServlet(...)");
        doGet(req, res);
    }
    ...
    throws IOException, ServletException
    {
        ...
        Object o = request.getParameter("includeThresholds");
        if ("true".equals(request.getParameter("includeThresholds")))
        {
        	thresholds = true;
        }
        logger.info("VFIChartMetricSwfDataServlet.doGet(...): includeThreshold = " + thresholds);
    ...
    throws IOException, ServletException
    {
        ...
        try
        {
            ...
            int counter = 0;
            Timestamp dPeriodId;
//            Double maxMetricValue, minMetricValue, sumMetricValue, movingAverageMetricValue, forecastValue, thresholdValue;
            Double sumMetricValue, forecastValue, thresholdValue;
            String value;
            while(rs.next())
            {
...
//                gid1 += "\n\t\t\t<value xid=\"" + counter + "\">" + value + "</value>";
                ...
                sumMetricValue = DatabasePersister.getDouble(rs, "sum_metric");
                value = sumMetricValue == null ? "" : sumMetricValue.toString();
                gid2 += "\n\t\t\t<value xid=\"" + counter + "\">" + value + "</value>";
...
//            gid1 += "\n\t\t</graph>";
            ...
            + gid8 + gid9 + gid10 + gid11 + gid12 + gid13 + gid14 + "\n\t</graphs>\n</chart>";
            osw.write(response);
        }
